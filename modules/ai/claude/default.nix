{
  ...
  mkSkillFiles = suffix:
    lib.listToAttrs (map (name:
      lib.nameValuePair ".config/claude${suffix}/skills/${name}" {
  ...
  karpathySkillText = ''
    ...
    **Define success criteria. Loop until verified.**
    ...
    Strong success criteria let you loop independently. Weak criteria ("make it work") require constant clarification.
  '';
  peonConfig = jsonFormat.generate "peon-ping-config" {
  ...
  mkSettings = peonshBin:
    jsonFormat.generate "claude-code-settings.json" {
        ...
        UserPromptSubmit = [
          ...
          }
          {
            ...
            hooks = [
              {
                type = "command";
                command = "${peonshBin}/bin/hook-handle-use";
                timeout = 5;
              }
            ];
          }
  ...
  # Claude Code wrapper for a config-dir suffix. Base ("") keeps the binary
  # name claude and ~/.config/claude, each suffix gets an isolated config dir
  # so separate accounts never share credentials or session state.
  mkClaude = suffix:
  ...
  # peon hook wrapper bound to a config dir. settings.json references it by
  # store path, so each instance needs its own so peon reads the right dir.
  mkPeonsh = suffix:
    pkgs.stdenv.mkDerivation {
      inherit (peon) pname version;
      dontUnpack = true;
  ...
  # Full per-instance config-dir population for a config-dir suffix.
  mkClaudeFiles = suffix: peonshBin:
    ...
    // {
      ...
      ".config/claude${suffix}/hooks/peon-ping/skills" = {
        source = "${peon}/share/peon-ping/skills";
        recursive = true;
      };
      ".config/claude${suffix}/hooks/peon-ping/packs" = {
        source = packs;
        recursive = true;
      };
      ".config/claude${suffix}/hooks/peon-ping/config.json" = {
        source = peonConfig;
      };
