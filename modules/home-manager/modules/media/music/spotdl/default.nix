  ...
  options = {
    modules = {
      media = {
        music = {
          spotdl = {
            enable = lib.mkEnableOption "Enable spotdl to download spotify playlists" // {default = false;};
          };
        };
      };
    };
  };
  config = lib.mkIf (cfg.enable && cfg.spotdl.enable) {
    home = {
      packages = [spotdl];
    };
    programs = {
      yt-dlp = {
        inherit (cfg.spotdl) enable;
      };
    };
    xdg = {
      dataFile = {
