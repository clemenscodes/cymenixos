  ...
  options = {
    modules = {
      display = {
        lockscreen = {
          swaylock = {
            enable = lib.mkEnableOption "Enable swaylock" // {default = false;};
          };
        };
      };
    };
  };
  config = lib.mkIf (cfg.enable && cfg.swaylock.enable) {
    home = {
    ...
    programs = {
      swaylock = {
        ...
        settings = let
          ...
          clear-color = "b5bd68";
        in
          lib.mkIf (!osConfig.modules.themes.catppuccin.enable) {
            clock = true;
